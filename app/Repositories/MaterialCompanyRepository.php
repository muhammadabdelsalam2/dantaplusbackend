...
class MaterialCompanyRepository
{
    ...
    {
        return MaterialCompany::query()
            ->leftJoin('material_orders', function ($join) {
                $join->on('material_orders.supplier_company_id', '=', 'material_companies.id')
                    ->where('material_orders.status', 'Completed');
            })
            ->select([
    ...
    public function commissionTotals(): array
    {
        ...
        return [
            'total_platform_commission' => (float) ($totals->total_commission_earned ?? 0),
            'total_platform_sales_completed' => (float) ($totals->total_sales ?? 0),
        ];
    }
}
